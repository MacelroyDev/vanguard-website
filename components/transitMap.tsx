...
export default function TransitMap({ config, className = '', adminMode = false }: TransitMapProps) {
  ...
  function getTrackId(path: Coordinate[]): string {
    const pathString = path.map(p => `${p.x.toFixed(1)},${p.y.toFixed(1)},${p.z.toFixed(1)}`).join('|');
    let hash = 0;
  ...
  }, [pan, zoom, trains, tracks, mergedStations, config, dimension, showAllStations, showWaypoints, showRivers, showMapOverlay, mapImage, trackStyles, selectedTrackIds, adminMode, showAdminPanel]);
  ...
  function drawTracks(
    ...
    filteredTracks.forEach(track => {
      ...
      const trackId = getTrackId(track.path);
    ...
    filteredTracks.forEach(track => {
      ...
      const style = trackStyles[trackId];
      const isHidden = style?.hidden;
      const trackColor = style?.color || '#d97706';
